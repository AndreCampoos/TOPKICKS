...
<body>
    ...
    <?php
        ...
        //Exibe o número de clientes/registos
        $nvendas=mysqli_num_rows($resultado);
        if($nvendas == 1){
            echo "<h3 class='nregistos pt-md-3'> Já foi efetuada $nvendas venda </h3>";
        }else{
            echo "<h3 class='nregistos pt-md-3'> Já foram efetuadas $nvendas vendas </h3>";
        }
    ?>
    
    <?php
        $consulta = "SELECT vendas.codvenda, clientes.nome, clientes.email, vendas.total, vendas.data FROM vendas INNER JOIN clientes ON vendas.codcliente = clientes.codcliente";
        $resultado=mysqli_query($liga,$consulta);
        $nVendas=mysqli_num_rows($resultado);
        ...
        echo "<h1 class='mt-md-3'>Vendas</h1>";
        echo "</div>";
        echo "</div>";
        echo "</div>";
        echo "<div class='horizontal_line mb-md-2 ml-md-0'></div>";

        echo "<h3 class='mt-md-4 text-right' style='margin-right: 7.3%; font-family: iran_sans_bold;'>Já foram efetuadas $nVendas vendas.</h3>";
