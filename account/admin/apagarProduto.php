...
if (isset($_GET['product_id'])) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['excluir'])) {
        ...
        if ($stmt) {
            mysqli_stmt_bind_param($stmt, "i", $product_id); //Se teve êxito vincula o parâmetro "i" com o valor do $product_id 
    ...
    $consultaProdutos = "SELECT * FROM produtos WHERE codigo = '$product_id'";
    $resultadoProdutos = mysqli_query($liga, $consultaProdutos);
    if ($resultadoCliente && mysqli_num_rows($resultadoProdutos) > 0) {
        $produto = mysqli_fetch_assoc($resultadoProdutos);
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>TOPKICKS - Apagar Produto</title>
...
<body>
    ...
    echo "<h1 class='mt-md-3'>Apagar Produto</h1>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
    ...
    for ($i = 0; $i < $nProdutos; $i++) {
        ...
        echo "<td>" . $registo['marca'] . " " . $registo['modelo'] . " " . $registo['submodelo'] . "</td>";
        echo "<td>";
        echo "<form method='POST' action='?product_id=" . $registo['codigo'] . "'>";
        echo '<button type="submit" name="excluir" class="ml-md-0" onclick="return confirm(\'Tem a certeza que pretende eliminar este produto?\');">';
